<template>
    <div id="pageAPIPermission" class="page-wrapper">
        <v-container grid-list-xl fluid>
                                ...
                                <template v-if="searchResult.list && searchResult.list.length > 0">
                                    <tr v-for="item in searchResult.list" :key="item.Id">
                                        ...
                                        <td>
                                            ...
                                            <v-icon size="20" color="deep-orange" @click="confirmDelete(item)">mdi-delete-forever</v-icon>
                                        </td>
                                    </tr>
                                </template>
                            </tbody>
            ...
            <v-dialog v-model="updateDialog.isShow" max-width="800px" persistent :disabled="saveLoading">
                <v-card ref="form">
                    <v-card-title>
                        <span class="headline mr-4">{{updateItem.Id === 0 ? '新建' : '编辑'}}</span><span>接口许可</span>
                    </v-card-title>
                    ...
                    <v-card-text>
                        <v-container>
                            ...
                            <v-row>
                                <v-text-field v-model="updateItem.LinkUrl" label="接口地址"
                                    :rules="[() => !!updateItem.LinkUrl || '不能为空.']" :error-messages="errorMessages"
                                    ref="Entity_LinkUrl">
                                </v-text-field>
                            </v-row>
                            <v-row>
                                <v-text-field v-model="updateItem.Sort" label="序号" ref="Entity_Sort"
            ...
            <v-dialog v-model="deleteDialog" persistent max-width="640">
                <v-card>
                    ...
                    <v-card-text>
                        ...
                        <p class="mb-1">接口地址：{{updateItem.LinkUrl}}</p>
                    </v-card-text>
                    <v-card-actions>
                        <v-spacer></v-spacer>
        ...
        methods: {
            ...
            saveUpdate: function () {
                ...
                if (this.updateItem.Id === 0) {
                    this.postAxios('/api/identity/backend/apipermission/create', JSON.stringify(this.updateItem)).then((data) => {
                        if (data.errorcode === 0) {
                            this.loadingDialog.isShow = false;
                    ...
                    this.postAxios('/api/identity/backend/apipermission/edit', JSON.stringify(this.updateItem)).then((data) => {
                        if (data.errorcode === 0) {
                            this.loadingDialog.isShow = false;
                            this.updateDialog.isShow = false;
            ...
            saveDelete: function () {
                ...
                this.getAxios('/api/identity/backend/apipermission/delete', { id: this.updateItem.Id, }).then((data) => {
                    if (data.errorcode === 0) {
                        this.loadingDialog.isShow = false;
            ...
            saveSetEnable: function () {
                ...
                this.getAxios('/api/identity/backend/apipermission/setenable', { id: this.updateItem.Id, }).then((data) => {
                    if (data.errorcode === 0) {
                        this.loadingDialog.isShow = false;
